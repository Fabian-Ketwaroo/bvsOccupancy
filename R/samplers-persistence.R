## Variable selection samplers for the persistence (phi) component.


#' Variable selection sampler for persistence
#'
#' Add / delete / swap Metropolis-Hastings update of the inclusion vector for the persistence (survival) component of a dynamic occupancy model. Only sites occupied in the current season contribute.
#'
#' @section Use:
...
#' Polya-Gamma update for persistence
#'
#' Draws the Polya-Gamma latent variables of the persistence component for a single season, at the sites occupied in that season.
#'
#' @section Use:
#' This sampler is assigned automatically by \code{\link{bvsSSOM}},
#' \code{\link{bvsSDOM}} and \code{\link{bvsMSOM}}. It is exported so that
...
PG_sampler_phi <- nimbleFunction(
  ...
  run = function() {
    ...
    for (i in 1:nsize) {
      model[[target]][xid[i]] <<- Omega_new[i]  
    }
...
#' Conjugate coefficient update for persistence
#'
#' Draws the active persistence coefficients from their Gaussian full conditional distribution given the Polya-Gamma latent variables.
#'
#' @section Use:
#' This sampler is assigned automatically by \code{\link{bvsSSOM}},
#' \code{\link{bvsSDOM}} and \code{\link{bvsMSOM}}. It is exported so that
